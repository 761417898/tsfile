...
class OrFilterTest : public ::testing::Test {
   protected:
    void SetUp() override {
        left_filter_ = new MockFilter();
        right_filter_ = new MockFilter();
        or_filter_ = new OrFilter(left_filter_, right_filter_);
    }
...
TEST_F(OrFilterTest, SatisfyStatistic) {
    ...
    EXPECT_TRUE(or_filter_->satisfy(&statistic));
}

TEST_F(OrFilterTest, SatisfyTimeValue) {
    EXPECT_CALL(*left_filter_, satisfy(123L, Object(456)))
        .WillOnce(testing::Return(false));
    EXPECT_CALL(*right_filter_, satisfy(123L, Object(456)))
        .WillOnce(testing::Return(true));

    EXPECT_TRUE(or_filter_->satisfy(123L, Object(456)));
}

TEST_F(OrFilterTest, SatisfyStartEndTime) {
    EXPECT_CALL(*left_filter_, satisfy_start_end_time(1000L, 2000L))
        .WillOnce(testing::Return(false));
    EXPECT_CALL(*right_filter_, satisfy_start_end_time(1000L, 2000L))
        .WillOnce(testing::Return(true));

    EXPECT_TRUE(or_filter_->satisfy_start_end_time(1000L, 2000L));
}

TEST_F(OrFilterTest, ContainStartEndTime) {
    EXPECT_CALL(*left_filter_, contain_start_end_time(1000L, 2000L))
        .WillOnce(testing::Return(false));
    EXPECT_CALL(*right_filter_, contain_start_end_time(1000L, 2000L))
        .WillOnce(testing::Return(true));

    EXPECT_TRUE(or_filter_->contain_start_end_time(1000L, 2000L));
}

TEST_F(OrFilterTest, GetTimeRanges) {
    ...
    std::vector<TimeRange*>* result = or_filter_->get_time_ranges();

    ASSERT_EQ(result->size(), 2);
    EXPECT_EQ(result->at(0)->start_time_, 1000L);
    EXPECT_EQ(result->at(0)->end_time_, 2500L);
    EXPECT_EQ(result->at(1)->start_time_, 3000L);
    EXPECT_EQ(result->at(1)->end_time_, 4000L);
